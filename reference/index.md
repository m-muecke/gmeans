# Package index

## All functions

- [`ad.test()`](https://m-muecke.github.io/gmeans/reference/ad.test.md)
  : Anderson-Darling Normality Test
- [`compute_wss()`](https://m-muecke.github.io/gmeans/reference/compute_wss.md)
  : Compute Within-Cluster Sum of Squares
- [`gmeans()`](https://m-muecke.github.io/gmeans/reference/gmeans.md) :
  G-means Clustering
- [`tidy(`*`<gmeans>`*`)`](https://m-muecke.github.io/gmeans/reference/gmeans_tidiers.md)
  [`augment(`*`<gmeans>`*`)`](https://m-muecke.github.io/gmeans/reference/gmeans_tidiers.md)
  [`glance(`*`<gmeans>`*`)`](https://m-muecke.github.io/gmeans/reference/gmeans_tidiers.md)
  : Tidy a G-means Clustering Object
- [`predict(`*`<gmeans>`*`)`](https://m-muecke.github.io/gmeans/reference/predict.gmeans.md)
  : Predict Method for G-means Clustering
- [`summary(`*`<gmeans>`*`)`](https://m-muecke.github.io/gmeans/reference/summary.gmeans.md)
  [`print(`*`<summary.gmeans>`*`)`](https://m-muecke.github.io/gmeans/reference/summary.gmeans.md)
  : Summarize a G-means Clustering Object
