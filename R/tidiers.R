...
#'
#' @description
#' Summarize a G-means clustering model as data frames, following the conventions of the broom
#' package. The generics come from the generics package, so load it or broom to call the methods.
#'
#' @param x (`gmeans()`)\cr
#'   An object of class `"gmeans"`.
#' @param col.names (`character()`)\cr
#'   Column names for the centers. Defaults to the column names of the centers, or `x1`, `x2`, ...
#'   if they are unnamed.
...
#' * `glance()`: one row with `k`, `k_init`, `k_max`, `level`, `totss`, `tot.withinss`, `betweenss`,
#'   and `iter`.
#' @seealso [gmeans()]
#' @name gmeans_tidiers
